...
abstract class BaseiceModelMultimediaQuery extends ModelCriteria
{
  ...
  public function findPk($key, $con = null)
  {
    ...
    if ((null !== ($obj = iceModelMultimediaPeer::getInstanceFromPool((string) $key))) && !$this->formatter)
    {
   ...
   *
   * @return    iceModelMultimedia A model object, or null if the key is not found
   */
  protected function findPkSimple($key, $con)
  {
    $sql = 'SELECT `ID`, `MODEL`, `MODEL_ID`, `TYPE`, `NAME`, `SLUG`, `MD5`, `SOURCE`, `IS_PRIMARY`, `POSITION`, `CREATED_AT` FROM `multimedia` WHERE `ID` = :p0';
    try
    {
    ...
    {
      $obj = new iceModelMultimedia();
      $obj->hydrate($row);
      iceModelMultimediaPeer::addInstanceToPool($obj, (string) $key);
    }
    $stmt->closeCursor();
